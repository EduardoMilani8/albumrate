...
- `app/` — rotas expo-router (`_layout.tsx`, `index.tsx`, `search.tsx`, `album/[id].tsx`, `diary.tsx`, `lists.tsx`, `list/[id].tsx`, `login.tsx`, `register.tsx`, `spotify-onboarding.tsx`, `profile.tsx`, `user/[id].tsx`, `album-of-month.tsx`, `album-of-month-history.tsx`)
- `components/` — `AlbumCard`, `StarRating`, `ReviewModal`, `MediaReviewCard`, `ListFormModal`, `AddToListModal`, `DailyPickCard` (botão/card "Álbum aleatório do dia" na home), `FeedItem` (item do feed de atividade), `DiversityChart` (donut de gêneros com react-native-svg), `WorldMap` (mapa-múndi de origens dos artistas; SVG puro do `react-native-svg`, cor = intensidade de álbuns, tooltip/destaque no toque, hover no web), `AlbumOfMonthAdminModal` (busca Spotify + escolha de mês/ano para o admin definir o álbum do mês)
- `constants/` — `theme.ts` (colors/spacing/radius dark; usar sempre)
- `lib/` — `db.ts` (SQLite local, só status `logged`/`want_to_listen`), `spotify.ts` (proxy de busca via backend `GET /api/spotify/search` — credenciais só no servidor), `spotifyAuth.ts` (OAuth PKCE manual: verifier S256, abre navegador), `useSpotifySignIn.ts` (hook do login Spotify + diálogo de conflito), `metadata.ts` (enriquecimento de gênero/ano/país via API pública do Deezer), `worldMapData.ts` (mapa SVG vendored `@svg-maps/world` CC-BY-4.0, 256 países, viewBox `0 0 1010 666`), `types.ts`, `api.ts` (cliente HTTP do backend), `auth.tsx` (AuthContext + SecureStore)
- `server/` — API Node/Express + Postgres (Drizzle): `src/schema.ts` (inclui tabelas `artists` — cache local de país de origem —, `daily_picks` — álbum sorteado do dia —, `follows` — seguidores/seguindo — e `album_of_month`/`album_of_month_comments` — álbum do mês), `src/routes/{auth,spotifyAuth,reviews,listeningLogs,lists,diversity,dailyPick,albumOfMonth,spotify,countries,social}.ts`, `src/lib/{dates,diversity,user,spotify,country,dailyPick,admin}.ts` (spotify.ts server-side: criptografia AES-256-GCM + exchange/refresh/imports/busca via Client Credentials; country.ts: normalização de país + lookup MusicBrainz + cache `artists`; dailyPick.ts: lógica de sorteio do álbum do dia; admin.ts: promoção automática via env `ADMIN_EMAILS`), `src/migrate.ts`, `drizzle/` (migrações geradas), `Dockerfile`, `.dockerignore`
...
- **Álbum do mês** (`album_of_month`, unique `month + year`; `album_of_month_comments`, FK cascade): pick **manual por admin** (`users.is_admin`; promoção automática via env `ADMIN_EMAILS` no login/cadastro/`/me`/Spotify). Metadados do álbum são denormalizados na tabela. Rotas: `GET /api/album-of-month` (atual), `GET /api/album-of-month/history` (mais recente primeiro), `GET /api/album-of-month/:id` (registrada DEPOIS de `/history`), `POST /api/album-of-month` (admin, upsert por mês — salvar de novo substitui), `GET/POST /api/album-of-month/:id/comments` (qualquer usuário logado; resposta expõe `user.id/name/avatarUrl`, nunca e-mail).
