...
- `app/` — rotas expo-router (`_layout.tsx` — providers + `useFonts` do `expo-font` com as famílias do design system; `index.tsx` — Home com header `albumrate.` próprio (sino/engrenagem placeholder), card compacto do álbum do dia com shuffle, card do álbum do mês com badge de votação, grid 3 colunas só com avaliados e sem FAB/filtros; `activity.tsx` — feed social, `search.tsx` (sem modal), `album/[id].tsx`, `diary.tsx`, `collection.tsx`, `lists.tsx`, `list/[id].tsx`, `login.tsx`, `register.tsx`, `spotify-onboarding.tsx`, `profile.tsx`, `appearance.tsx`, `user/[id].tsx`, `album-of-month.tsx`, `album-of-month-history.tsx`)
