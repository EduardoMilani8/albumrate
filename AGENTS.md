...
- `components/` — `AlbumCard`, `StarRating`, `ReviewModal`, `MediaReviewCard`, `ListFormModal`, `AddToListModal`, `DiversityChart` (donut de gêneros com react-native-svg), `WorldMap` (mapa-múndi de origens dos artistas; SVG puro do `react-native-svg`, cor = intensidade de álbuns, tooltip/destaque no toque, hover no web)
...
- `server/` — API Node/Express + Postgres (Drizzle): `src/schema.ts` (inclui tabela `artists` — cache local de país de origem), `src/routes/{auth,spotifyAuth,reviews,listeningLogs,lists,diversity,spotify,countries}.ts`, `src/lib/{dates,diversity,user,spotify,country}.ts` (spotify.ts server-side: criptografia AES-256-GCM + exchange/refresh/imports/busca via Client Credentials; country.ts: normalização de país + lookup MusicBrainz + cache `artists`), `src/migrate.ts`, `drizzle/` (migrações geradas), `Dockerfile`, `.dockerignore`
