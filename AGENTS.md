...
- `app/` — rotas expo-router (`_layout.tsx`, `index.tsx`, `search.tsx`, `album/[id].tsx`, `diary.tsx`, `collection.tsx`, `lists.tsx`, `list/[id].tsx`, `login.tsx`, `register.tsx`, `spotify-onboarding.tsx`, `profile.tsx`, `appearance.tsx`, `user/[id].tsx`, `album-of-month.tsx`, `album-of-month-history.tsx`)
...
- `constants/` — `theme.ts` (tokens do tema padrão dark; `spacing`/`radius`; usar sempre), `themes.ts` (5 temas: Claro, Escuro, Midnight, Vinil Sépia, Contraste Neon, cada um com todos os tokens de cor)
