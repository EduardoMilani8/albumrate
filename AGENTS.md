...
- `server/` — API Node/Express + Postgres (Drizzle): `src/schema.ts` (inclui tabelas `artists` — cache local de país de origem — e `daily_picks` — álbum sorteado do dia), `src/routes/{auth,spotifyAuth,reviews,listeningLogs,lists,diversity,dailyPick,spotify,countries}.ts`, `src/lib/{dates,diversity,user,spotify,country,dailyPick}.ts` (spotify.ts server-side: criptografia AES-256-GCM + exchange/refresh/imports/busca via Client Credentials; country.ts: normalização de país + lookup MusicBrainz + cache `artists`; dailyPick.ts: lógica de sorteio do álbum do dia), `src/migrate.ts`, `drizzle/` (migrações geradas), `Dockerfile`, `.dockerignore`
